...
mod tests {
    ...
    fn test_success_deposit_cw20() {
        ...
        let available_to_borrow_cw20_before_failed_borrow: Uint128 = app
            .wrap()
            .query_wasm_smart(
                lending_addr.clone(),
        ...
        let cw20_user_balance_before_failed_borrow: BalanceResponse = app
            .wrap()
            .query_wasm_smart(
                cw20_token_addr.clone(),
                &Cw20QueryMsg::Balance {
        ...
        assert!(app
            .execute_contract(
                ...
                &ExecuteMsg::Borrow {
                    denom: "ilend-denom".to_string(),
                    amount: Uint128::from(available_to_borrow_cw20_before_failed_borrow.u128() * 2),
                },
                &[],
            )
            .is_err());

        let cw20_user_balance_after_failed_borrow: BalanceResponse = app
            .wrap()
            .query_wasm_smart(
                cw20_token_addr.clone(),
                &Cw20QueryMsg::Balance {
        ...
        assert_eq!(
            cw20_user_balance_before_failed_borrow.balance.u128(),
            cw20_user_balance_after_failed_borrow.balance.u128()
        );

        let available_to_borrow_cw20_after_failed_borrow: Uint128 = app
            .wrap()
            .query_wasm_smart(
                lending_addr.clone(),
                &QueryMsg::GetAvailableToBorrow {
        ...
        assert_eq!(
            available_to_borrow_cw20_after_failed_borrow.u128(),
            available_to_borrow_cw20_before_failed_borrow.u128()
        );
    }
}
