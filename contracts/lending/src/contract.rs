...
use crate::state::{IS_TESTING, LIQUIDITY_INDEX_DATA, PRICES, PRICE_FEED_IDS, PRICE_UPDATER_CONTRACT, PYTH_CONTRACT, TOTAL_BORROW_DATA, USER_BORROWING_INFO, USER_DEPOSIT_AS_COLLATERAL, MARGIN_POSITIONS_CONTRACT, COLLATERAL_VAULT_CONTRACT};
...
use {
    ...
    crate::{
        ...
        state::{
            ADMIN, LIQUIDATOR, RESERVE_CONFIGURATION, SUPPORTED_TOKENS,
...
pub fn instantiate(
    ...
    COLLATERAL_VAULT_CONTRACT.save(deps.storage, &msg.collateral_vault_contract)?;
    MARGIN_POSITIONS_CONTRACT.save(deps.storage, &msg.margin_positions_contract)?;
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Redeem { denom, amount } => {
            ...
            let msg_redeem_from_vault_contract = RedeemFromVaultContract {
                denom: denom.clone(),
            ...
            Ok(
                Response::new().add_message(CosmosMsg::Wasm(WasmMsg::Execute {
                    ...
                    msg: to_binary(&msg_redeem_from_vault_contract)?,
                    funds: vec![],
                })),
            )
        }
        ...
        ExecuteMsg::Borrow { denom, amount } => {
            ...
            if info.sender.to_string() == get_margin_positions_contract(deps.as_ref()).unwrap() {
                assert!(
                    get_available_liquidity_by_token(deps.as_ref(), denom.clone())
                        .unwrap()
                        .u128()
                        >= amount.u128()
                );
            ...
            // initiating borrow from vault contract if all checks are passed
            Ok(
                Response::new().add_message(CosmosMsg::Wasm(WasmMsg::Execute {
                    contract_addr: get_collateral_vault_contract(deps.as_ref()).unwrap(),
                    msg: to_binary(&BorrowFromVaultContract {
                        denom: denom.clone(),
                        amount: Uint128::from(amount),
                        user: info.sender.to_string(),
                    })?,
            ...
            if remaining_amount > 0 {
                Ok(Response::new()
                    ...
                    .add_message(BankMsg::Send {
                        to_address: get_collateral_vault_contract(deps.as_ref()).unwrap(),
                        amount: coins(
                            repay_token.amount.u128() - remaining_amount,
                            repay_token.denom.clone(),
                        ),
                    }))
            } else {
                Ok(Response::new().add_message(BankMsg::Send {
                    to_address: get_collateral_vault_contract(deps.as_ref()).unwrap(),
                    amount: coins(repay_token.amount.u128(), repay_token.denom.clone()),
                }))
            }
        }
        ...
        ExecuteMsg::SetCollateralVaultContract { contract } => {
            ...
            COLLATERAL_VAULT_CONTRACT.save(deps.storage, &contract)?;
            Ok(Response::default())
        }
    }
}
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetAvailableLiquidityByToken { denom } => {
            to_binary(&get_available_liquidity_by_token(deps, denom)?)
        }
        ...
        QueryMsg::GetAllUsersWithBorrows {} => to_binary(&get_all_users_with_borrows(deps)?),
        QueryMsg::GetCollateralVaultContract {} => to_binary(&get_collateral_vault_contract(deps)?),
        QueryMsg::GetMarginPositionsContract {} => to_binary(&get_margin_positions_contract(deps)?),
    }
}
...
pub mod query {
    ...
    use crate::state::{COLLATERAL_VAULT_CONTRACT, MARGIN_POSITIONS_CONTRACT};
    ...
    pub fn get_collateral_vault_contract(deps: Deps) -> StdResult<String> {
        COLLATERAL_VAULT_CONTRACT.load(deps.storage)
    }

    pub fn get_margin_positions_contract(deps: Deps) -> StdResult<String> {
    ...
    pub fn get_available_liquidity_by_token(deps: Deps, denom: String) -> StdResult<Uint128> {
        let coins: Vec<Coin> = deps
            .querier
            .query_all_balances(get_collateral_vault_contract(deps).unwrap())?;
    ...
    pub fn get_available_to_borrow(
        ...
        if max_allowed_borrow_amount_usd > sum_user_borrow_balance_usd {
            ...
            let token_liquidity = get_available_liquidity_by_token(deps, denom.clone())
                .unwrap()
                .u128();
    ...
    pub fn get_all_users_with_borrows(deps: Deps) -> StdResult<Vec<String>> {
