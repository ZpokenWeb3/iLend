...
use crate::state::{
    IS_TESTING, LIQUIDITY_INDEX_DATA, PRICES, PRICE_FEED_IDS, PRICE_UPDATER_ADDRESS,
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdatePrice { denom, price } => {
            ...
            // if not Testing mode, PRICE_UPDATER_ADDRESS fetching price from Pyth contract, if not available,  leaving as is
            if IS_TESTING.load(deps.storage).unwrap() {
                ...
                if price.unwrap() == 0 {
                    for token in get_supported_tokens(deps.as_ref())
                        .unwrap()
                        .supported_tokens {
                        let price = fetch_price_by_token(deps.as_ref(), env.clone(), token.denom.clone())
                            .unwrap()
                            .u128();
