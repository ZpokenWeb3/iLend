...
use lending::msg::{
    InstantiateMsg as InstantiateMsgLending,
};

use lending::{
    execute as execute_lending, instantiate as instantiate_lending,
    query as query_lending,
};
...
pub fn success_setup_collateral_vault_and_margin_contract() -> (BasicApp<CustomMsg>, Addr, Addr) {
    ...
    app.execute_contract(
        Addr::unchecked("collateral_vault"),
        collateral_contract_addr.clone(),
        &ExecuteMsgCollateralVault::SetMarginPositionsContract {
...
pub fn success_setup_three_contracts() -> (BasicApp<CustomMsg>, Addr, Addr, Addr) {
    const TOKENS_DECIMALS: u32 = 18;
    ...
    let margin_positions_addr = app
        .instantiate_contract(
            ...
            &InstantiateMsgMarginPositions {
                ...
                pyth_contract_addr: "inj1z60tg0tekdzcasenhuuwq3htjcd5slmgf7gpez".to_string(),
                admin: "margin_positions".to_string(),
    ...
    let code = ContractWrapper::new_with_empty(execute_lending, instantiate_lending, query_lending);
    let code_id = app.store_code(Box::new(code));

    let lending_contract_addr = app
        .instantiate_contract(
            code_id,
            Addr::unchecked("lending"),
            &InstantiateMsgLending {
                is_testing: true,
                price_ids: vec![
                    (
                        "inj".to_string(),
                ...
                price_updater_contract_addr: "".to_string(),
                collateral_vault_contract: collateral_contract_addr.to_string(),
                margin_positions_contract: margin_positions_addr.to_string(),
            },
            &[],
            "Contract",
    ...
    app.execute_contract(
        Addr::unchecked("collateral_vault"),
        collateral_contract_addr.clone(),
        &ExecuteMsgCollateralVault::SetLendingContract {
            contract: lending_contract_addr.to_string(),
        },
        &[],
    )
        .unwrap();
    ...
    (app, lending_contract_addr, margin_positions_addr, collateral_contract_addr)
}
